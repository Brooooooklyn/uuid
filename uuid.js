  ...
  case 'linux':
    switch (arch) {
      ...
      case 'riscv64':
        if (isMusl()) {
          localFileExisted = existsSync(
            join(__dirname, 'uuid.linux-riscv64-musl.node')
          )
          try {
            if (localFileExisted) {
              nativeBinding = require('./uuid.linux-riscv64-musl.node')
            } else {
              nativeBinding = require('@napi-rs/uuid-linux-riscv64-musl')
            }
          } catch (e) {
            loadError = e
          }
          ...
          localFileExisted = existsSync(
            join(__dirname, 'uuid.linux-riscv64-gnu.node')
          )
          try {
            if (localFileExisted) {
              nativeBinding = require('./uuid.linux-riscv64-gnu.node')
            } else {
              nativeBinding = require('@napi-rs/uuid-linux-riscv64-gnu')
            }
          } catch (e) {
            loadError = e
          }
...
if (!nativeBinding || process.env.NAPI_RS_FORCE_WASI) {
  try {
    localFileExisted = existsSync(
      join(__dirname, 'uuid.wasm32-wasi.wasm')
    ) && existsSync(join(__dirname, 'uuid.wasi.cjs'))
    if (localFileExisted) {
